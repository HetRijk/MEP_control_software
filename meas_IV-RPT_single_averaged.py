...
Averages IV curve point over number of measurements
"""
...
meas_name = 'WO3196dev1_IVcurve_100' 
...
for n, i in enumerate(source_currents):       
    ...
    av_current.append([np.mean(current[0][-num_meas_av:]), np.mean(current[1][-num_meas_av:])])
    av_current.append([np.mean(current[0][-num_meas_av:]), np.mean(current[1][-num_meas_av:])])
...
# IV Curve
plt.figure(6)
plt.plot(av_current[1], av_volt[1])
plt.title('IV Curve averaged over %s measurements per point' % num_meas_av)
plt.xlabel('Current (nA)')
plt.ylabel('Voltage (V)')
plt.grid()

instr.save_plot('%s\%s_ivcurve_avrg' % (figure_folder, meas_name))
